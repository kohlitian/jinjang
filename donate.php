...
if($_SERVER['REQUIEST_METHOD'] == 'POST'){
	...
	if($amount<=0){
		$amountError = "Sorry amount cannot be less than or equal to 0.";
	} else {
		...
		bootbox.confirm({message: "Are you sure you want to donate " + amount + " for us ?", 
			...
			Callback: function(result){
				header("Location: afterDonate.php");
			}});
	}
}
		...
		<form class="col-xs-offset-1 col-xs-10 col-sm-offset-2 col-sm-8 col-md-offset-3 col-md-6 col-lg-offset-3 col-lg-6" method="POST" action="LogIn.php">
			<div class="formStyle">
				...
				<div class="input-group">
					...
					<input type="text" required class="form-control" placeholder="Please enter your full name (can be empty)" id="fullName" name="fullName" value="<?php if(isset($user['fullName'])){echo $user['fullName'];} ?>">
				</div>
				<div class="input-group">
					<span class="input-group-addon" id="basic-addon2"><label for="email"><i class="fa fa-key"></i></label></span>
					<input class="form-control" required placeholder="Please enter your email" type="email" id="email" name="email" value="<?php if(isset($user['email'])){echo $user['email'];} ?>">
				</div>
				<div class="input-group">
					<span class="input-group-addon" id="basic-addon2"><label for="contactNo"><i class="fa fa-key"></i></label></span>
					<input class="form-control" required placeholder="Please enter your contact Number" type="tel" id="contactNo" name="contactNo" value="<?php if(isset($user['contactNo'])){echo $user['contactNo'];} ?>">
				</div>
